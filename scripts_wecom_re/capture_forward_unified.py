"""
capture_forward_unified.py
单脚本：Frida hook（系统 Python 311）+ subprocess 调用 UI 自动化（venv Python）
用 subprocess 异步运行 UI 脚本，主线程运行 Frida
"""
import sys, time, json, subprocess, threading, frida
...
def on_msg(msg, data):
    ...
    t = p.get("t","")
    if t == "ready":
        print("[Frida] Hook READY!", flush=True)
        hook_ready.set()
    elif t == "insert":
        events.append(p)
        strs = p.get("strs", [])
...
if not hook_ready.wait(timeout=5):
    print("Hook 未就绪!", flush=True)
    sys.exit(1)

# 启动 UI 进程
VENV_PY = str(ROOT / ".venv" / "Scripts" / "python.exe")
UI_SCRIPT = str(ROOT / "scripts" / "wecom_re" / "forward_ui_proc.py")

print(f"\n[UI] 启动 UI 自动化进程: {VENV_PY}", flush=True)
...
def drain_ui():
    for line in ui_proc.stdout:
        print(f"  [UI] {line.rstrip()}", flush=True)

drain_t = threading.Thread(target=drain_ui, daemon=True)
drain_t.start()

# 等 UI 进程结束 (最多 60s)
try:
    ui_proc.wait(timeout=60)
except subprocess.TimeoutExpired:
    print("[!] UI 进程超时", flush=True)
    ui_proc.kill()

drain_t.join(timeout=2)
print(f"[UI] 进程结束，等 3s 让 DB 写入完成...", flush=True)
time.sleep(3)
...
print(f"\n{'='*60}", flush=True)
print(f"总 INSERT 事件: {len(events)}", flush=True)
print(f"新(非背景)事件: {len(new_events)}", flush=True)

if new_events:
    print("\n🎯 转发专属 INSERT 已捕获！", flush=True)
